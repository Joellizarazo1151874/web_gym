<?php
/**
 * Actualizar horario de clase para Aplicación Móvil
...
try {
    ...
    if (!$auth->hasRole(['admin', 'entrenador'])) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'No autorizado. Solo administradores y entrenadores pueden actualizar horarios.'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    // Si es entrenador, solo puede actualizar horarios de sus propias clases
    ...
    // Verificar si ya existe otro horario para este día y hora (excluyendo el actual)
    $stmt = $db->prepare("
    ...
    error_log("Error en mobile_update_class_schedule.php: " . $e->getMessage());
